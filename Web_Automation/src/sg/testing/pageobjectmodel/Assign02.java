...
class Modifyuser
{
	...
	static void usermodify()
	{
		try
		{
			oPage.getclickonuserTaskbutton().click();
			Thread.sleep(2000);
			oPage.getuserusernameInAdduser().sendKeys("Kohli");
			Thread.sleep(2000);
			oPage.getclickonuseridsavebutton().click();
			Thread.sleep(2000);
...
public class Assign02 {
	...
	public static void main(String[] args) {
		...
		Modifyuser.launchBrowser();
		Modifyuser.navigate();
		Modifyuser.login();
		Modifyuser.minimizeFlyOutWindow();
		Modifyuser.Createuser();
		Modifyuser.usermodify();
		Modifyuser.Deleteuser();
		Modifyuser.logout();
		Modifyuser.closeApplication();
